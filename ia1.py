...
def target(maze, index, ephs, pers):
    # if ephs != []:
    #     closest = Astar_search(maze, index, ephs[0])
    #     min = len(closest)
    #     for e in ephs:
    ...
    # return closest
    resources = ephs + pers
    first = Astar_search(maze, index, resources[0])
    return first if first != [] else choice(check_valids(maze, index))
...
def Astar_search(maze, start, target):
    ...
    if road == []:
        return [start, check_valids(maze, start)[0]]
